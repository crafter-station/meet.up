...
- **Realtime events**: `message:add`, `messages:sync`, `partial:update`, `meeting:ended`
- **Hooks**: `useRealtimeChat()` (messaging + meeting end), `useTranscription()` (ElevenLabs scribe)

## Database tables

- **rooms**: id, dailyRoomName (unique), dailyRoomUrl, createdAt, expiresAt, endedAt
